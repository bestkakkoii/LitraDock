...
Go net/http and pgx implement account/session admission, tenant-scoped libraries, genuine PubMed search, durable selected batches, permitted PMC OAI XML originals, per-item reasons, UTF-8 CSV and ZIP export. React/TypeScript/Vite compiles to static files served directly by Go. Neither .NET nor Node is required at runtime. This is an isolated qualification candidate, not public native cutover or complete research/import/export parity.
...
Stop before paired backup; schema1 keeps metadata and original bytea in one PostgreSQL snapshot. Restore first into an isolated database and compare identities, hashes and paused work. Before exposing any restored account state, revoke restored sessions; a supported native restore/session-revocation operator command is still pending. Keep code/config/DB backup identities together, and never run an older schema writer against a newer schema without explicit compatibility qualification. Public TLS/cutover, native import/research recovery, practical bulk, typed XLSX, two-positive-source qualification, off-host backup/alerts, physical devices, clean install, signing and current vulnerability clearance remain open.
