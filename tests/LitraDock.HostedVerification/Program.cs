...
else if (args.FirstOrDefault() == "--credential-rotation")
{
    await CredentialRotationChecks.Run(Environment.GetEnvironmentVariable("LITRADOCK_PG_TEST_CONNECTION"), output, Check);
}
...
else if (args.FirstOrDefault() == "--derived-identity")
{
    await DerivedIdentityChecks.Run(output, Check);
}
