...
try:
    with (out/'server.log').open('wb') as server_log:
        ...
        with (out/'browser.log').open('wb') as log:
            ...
            subprocess.run([npm, 'test'], cwd=repo/'tests/native-browser', env=env, stdout=log, stderr=subprocess.STDOUT, timeout=480, check=True, **flags)
        Path(str(input_file)+'.stop').touch()
        if process.wait(timeout=20) != 0:
            raise RuntimeError('Test server exit failed')
        receipt = {'revision': args.revision, 'scope': 'Actual native PostgreSQL/compiled handlers/static frontend; exclusively synthetic source transport, no live coverage', 'test_binary_sha256': hashlib.sha256(binary.read_bytes()).hexdigest(), 'manifest_sha256': hashlib.sha256(args.manifest.read_bytes()).hexdigest(), 'browser_exit': 0, 'server_exit': 0}
