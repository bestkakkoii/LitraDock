...
function requireAuthenticationGeneration(generation) {
  if (generation !== authenticationGeneration) throw new Error("Account context changed; request discarded.");
}
function privateResponse(response, generation) {
  ...
  return {ok:response.ok,status:response.status,headers:response.headers,
    json:()=>read("json"),blob:()=>read("blob"),text:()=>read("text"),
    clone:()=>privateResponse(response.clone(),generation)};
}
async function privateFetch(path, options) {
  const generation = authenticationGeneration;
  const response = await fetch(path,options);
  // 舊帳號的 401 不能登出之後登入的新帳號；每次 headers/body 完成均明確檢查世代。
  requireAuthenticationGeneration(generation);
  observeAuthentication(response);
  requireAuthenticationGeneration(generation);
  return privateResponse(response,generation);
}
...
async function progress() {
  ...
  for (const i of r.items) {
    ...
    row.append(open);
    if (!demoMode && !["queued", "running"].includes(r.state)) {
...
$("restoreBundle").onclick=safe(async()=>{
  ...
  try {
    ...
    for(let attempt=0;attempt<4;attempt++) {
      ...
      response=await privateFetch("/api/restore",{method:"POST",headers:{"Content-Type":"application/octet-stream","X-CSRF":initialCsrf},body:file});
...
try {
  const service = await (await fetch("/service-info")).json();
  $("softwareSource").href = service.source;
  if(service.demo) {
    demoMode=true;
    ...
    for(const id of ["export","preview","csv"]) { if($(id) && id!=="csv")$(id).hidden=true; }
  }
} catch {$("serviceInformation").textContent="Service information could not be loaded; contact the operator before using this preview.";}
