...
export function PlanExports(props: Props) {
  ...
  useLayoutEffect(() => {
    ...
    const retire = () => { controller.abort(); operation.current++; inFlight.current = false; setBusy(null); setError(""); setNotice(""); };
    ...
    return () => {
      controller.abort(); operation.current++;
  ...
  const save = async (format: PlanExportFormat) => {
    ...
    const current = () => !signal.aborted && id === operation.current && props.generation === sessionGeneration();
    if (!current()) return;
  ...
  return <section className="plan-exports" aria-label="Saved plan exports">
    ...
    {busy && <p role="status">Preparing plan {busy === "zip" ? "originals ZIP" : "metadata JSON"}…</p>}
