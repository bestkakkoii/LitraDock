...
export function ArticleCard({ article, selected, onSelect }: { article: Article; selected: boolean; onSelect: () => void }) {
  ...
  return <article className="result-card">
    ...
    <nav className="links" aria-label="Source links">{links.map(([name, value]) => { const href = name === "DOI" && doiBlocked ? null : safeLink(value); return href ? <a key={name} href={href} target="_blank" rel="noopener noreferrer">Open {name}</a> : null; })}</nav>
