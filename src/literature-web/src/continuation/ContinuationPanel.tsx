...
export function ContinuationPanel({ status, state, controller, runID, visible, checked }: {
  ...
  const blocked = state.busy || !!state.pending || !!state.confirmed;
  return <section className="continuation" aria-label="Search continuation">
    ...
    {status ? <>
      ...
      <details><summary>Membership and retrieval limits</summary>
        ...
        <p>New saved records do not expand checked selection, the basket or an existing download plan. Refreshing or reopening only reads saved status.</p>
      </details>
