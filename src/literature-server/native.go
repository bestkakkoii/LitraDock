...
func nativeOperator(ctx context.Context, verb string) error {
	...
	if verb == "bootstrap" {
		...
		var version int
		if e = tx.QueryRow(ctx, "SELECT max(version) FROM native_schema").Scan(&version); e != nil || (version != 3 && version != 4 && version != 5 && version != 6 && version != 8 && !(verb == "transition-shared-trial" && (version == 1 || version == 2))) {
